...
def validate_target_weights(targets: pd.DataFrame) -> pd.DataFrame:
    ...
    if validated["target_weight"].isna().any() or (validated["target_weight"] <= 0).any():
...
def select_equal_weight_targets(candidates: pd.DataFrame, holding_count: int) -> pd.DataFrame:
