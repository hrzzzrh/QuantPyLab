...
from backtest.data_access import IndicatorField
...
class QualityValueRecoveryStrategy(BacktestStrategy):
    ...
    def load_signal_data(self, data_access, config, parameters) -> pd.DataFrame:
        ...
        return data_access.load_market_data(
            config, lookback_days, self.indicator_fields
        )
    ...
    def build_targets(self, signal_data, config, parameters) -> pd.DataFrame:
        ...
        rebalance_data = rebalance_data[
            ...
            & (
                rebalance_data["operating_cashflow_to_revenue"]
                > parameters["min_operating_cashflow_to_revenue"]
            )
            & (rebalance_data["close_hfq"] > rebalance_data["trend_average"])
        ].copy()
        rebalance_data["pe_percentile"] = rebalance_data.groupby("date")["pe_ttm"].rank(
            pct=True
        )
        rebalance_data["pb_percentile"] = rebalance_data.groupby("date")["pb"].rank(
            pct=True
        )
