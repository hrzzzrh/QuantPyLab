...
- [x] **财务数据同步 engine**：实现基于“披露日历驱动”的智能增量同步，支持 A 股三大财务报表的采集。
...
- **功能完备性 (Definition of Done)**：在宣布任务完成前，**必须显式执行以下清单**，确保没有遗留碎片：
    ...
    3. **精准暂存 (Precise Staging)**：对新增或修改的文档及代码文件执行精准的 `git add <file>`。**严禁执行 `git add .`、`git add -A` 或任何 `git commit` 操作**。
    4. **文档同步 (Doc Sync)**：更新 `README.md` 和 `docs/` 下的所有受影响文档（如 `usage.md`, `architecture.md`, `data_schema.md`），确保文档始终反映最新的代码现状。
    5. **进度更新 (Progress Updates)**：在 `GEMINI.md` 的“4. 项目进度”中记录完成情况，并同步更新“5. 后续计划”。
    6. **交付通告 (Handover)**：在对话中明确列出建议用户提交的文件清单，并提供符合规范的 Commit Message 建议。
- **刷新机制**：修改本文件后，提醒执行 `/memory refresh`。

## 7. Git 协作规范
- **权限边界**：AI **禁止**执行 `git commit` 和 `git push`。AI 被授权在 DoD 阶段对受影响的文件执行精准的 `git add`。
- **提交建议**：AI 必须为每次 Feature 完成提供建议的 Commit Message，格式遵循 Conventional Commits (例如 `feat: ...`, `fix: ...`)。

## 8. 技术方案编写规范 (RFC Guidance)
