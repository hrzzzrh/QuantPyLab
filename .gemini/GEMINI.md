...
## 3. 开发核心原则
1. **文档驱动**：修改或增加核心逻辑前，先在 `workspace/` 更新设计思路。**开发过程必须严格遵循设计文档执行。**
2. **分析优先**：利用 DuckDB 列式特性进行 SQL 层筛选，减少内存开销。
3. **读写分离**：统一通过 `storage/manager.py` 接口操作数据。
4. **探索与验证**：在正式设计前，应在 `workspace/` 编写测试 Demo（如 API 探测、逻辑验证），基于真实反馈修正设计。**严禁在未经验证的情况下凭空设计。** 所有探测、demo、测试代码必须写在 workspace 目录下，禁止以其他方式写临时代码。
