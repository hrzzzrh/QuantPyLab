    ...
    def fetch_all_etfs(self) -> pd.DataFrame:
        ...
        try:
            ...
            if "成立日期" in df_cleaned.columns:
                df_cleaned["list_date"] = pd.to_datetime(
                    df_cleaned["成立日期"], errors="coerce"
                ).dt.strftime("%Y%m%d")
            else:
...
class ETFKlineCollector:
    ...
    def _get_local_max_date(self, symbol: str) -> str:
        ...
        try:
            path = (
                self.store.base_dir / "etf_kline" / f"symbol={symbol}" / "data.parquet"
            )
            if not path.exists():
                return "19900101"
    ...
    def _fetch_from_xueqiu(
        self, symbol: str, type_param: str, count: int = -10000
    ) -> pd.DataFrame:
        """
        从雪球获取K线数据
        ...
        if "data" not in data:
            raise Exception(
                f"雪球接口返回数据格式异常: {data.get('error_description', 'Unknown error')}"
            )
    ...
    def collect_kline(self, symbol: str, start_date: str = None, end_date: str = None):
        ...
        try:
            ...
            df_normal = self._fetch_from_xueqiu(
                xq_symbol, type_param="normal", count=-10000
            )
            ...
            df_after = self._fetch_from_xueqiu(
                xq_symbol, type_param="after", count=-10000
            )
