...
class ShareCollector:
    ...
    def _get_local_max_date(self, symbol: str) -> str:
        ...
        try:
            ...
            res = conn.execute(
                f"SELECT MAX(change_date) FROM read_parquet('{path}')"
            ).fetchone()
            if res and res[0]:
    ...
    def _fetch_sina_share_capital(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(
                    f"请求新浪股本页面失败: {symbol}, Status: {response.status_code}"
                )
                    ...
                    if len(tds) >= 2:
                        ...
                        if re.match(r"\d{4}-\d{2}-\d{2}", date_str):
                            ...
                            if shares_match:
                                # 转换为股并使用 int64
                                shares = int(
                                    round(float(shares_match.group(1)) * 10000)
                                )
                                data.append(
                                    {"change_date": date_str, "total_shares": shares}
                                )
            ...
            if not df.empty:
                ...
                df["symbol"] = symbol
                df = df.sort_values("change_date", ascending=True).drop_duplicates(
                    "change_date"
                )
            return df
    ...
    def _save_incremental(self, df_new: pd.DataFrame, symbol: str):
        """增量合并并保存"""
        path = (
            self.store.base_dir / "share_capital" / f"symbol={symbol}" / "data.parquet"
        )
        ...
        if path.exists():
            ...
            df_combined.drop_duplicates(
                subset=["change_date"], keep="last", inplace=True
            )
        ...
        try:
            df_combined["total_shares"] = (
                df_combined["total_shares"].astype(float).round().astype("int64")
            )
        except Exception as e:
            logger.warning(f"转换 {symbol} 股本为 int64 失败: {e}，尝试强制转换")
            df_combined["total_shares"] = (
                pd.to_numeric(df_combined["total_shares"], errors="coerce")
                .fillna(0)
                .astype("int64")
            )
