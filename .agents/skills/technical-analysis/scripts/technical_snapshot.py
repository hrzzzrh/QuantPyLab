...
用法:
    PYTHONPATH=. uv run python .agents/skills/technical-analysis/scripts/technical_snapshot.py <symbol> --output <path.md>
"""
...
def main():
    ...
    try:
        ...
        hist_52w = hist[
            hist["date"]
            >= hist["date"].max() - __import__("pandas").Timedelta(days=365)
        ]
        ...
        if n >= 20:
            std20 = float(
                closes_hfq[-20:][0]
                and (sum((x - bb_mid) ** 2 for x in closes_hfq[-20:]) / 20) ** 0.5
            )
        ...
        lines.append("> 统计口径：趋势指标均为后复权价，不复权价仅用于标注")
        lines.append("")
        lines.append("## 一、基础信息")
        lines.append("")
        lines.append("| 项目 | 数值 |")
        ...
        for v, label in vals:
            ...
            lines.append(f"| {label} | {v:.2f} 元 | {conv_v:.2f} 元 | {pct:+.1f}% |")
        lines.append("")
        lines.append("## 三、历史极值")
        lines.append("")
        lines.append("| 项目 | 日期 | 后复权价 | 折算不复权（约） |")
        lines.append("|:---|:---:|:---:|:---:|")
        lines.append(
            f"| 历史最高 (ATH) | {ath_row['date'].strftime('%Y-%m-%d')} | {ath_row['close_hfq']:.2f} 元 | {conv(ath_row['close_hfq']):.2f} 元 |"
        )
        lines.append(
            f"| 52周最高 | {hi52['date'].strftime('%Y-%m-%d')} | {hi52['close_hfq']:.2f} 元 | {conv(hi52['close_hfq']):.2f} 元 |"
        )
        lines.append(
            f"| 52周最低 | {lo52['date'].strftime('%Y-%m-%d')} | {lo52['close_hfq']:.2f} 元 | {conv(lo52['close_hfq']):.2f} 元 |"
        )
        lines.append("")
        ...
        lines.append(
            f"| 布林上轨(20,2) | {bb_up:.2f} |" if bb_up else "| 布林上轨 | 数据不足 |"
        )
        lines.append(
            f"| 布林中轨(20) | {bb_mid:.2f} |" if bb_mid else "| 布林中轨 | 数据不足 |"
        )
        lines.append(
            f"| 布林下轨(20,2) | {bb_low:.2f} |"
            if bb_low
            else "| 布林下轨 | 数据不足 |"
        )
        lines.append("")
        ...
        lines.append(
            f"| 放量倍数（今/均） | {today_vol / avg_vol:.2f}x |"
            if avg_vol
            else "| 放量倍数 | - |"
        )
        lines.append(
            f"| 区间最大量 | {vol_max['volume']:.0f} 手（{vol_max['date'].strftime('%Y-%m-%d')}，收盘 {vol_max['close_hfq']:.2f} 后复权） |"
        )
        lines.append(
            f"| 区间最小量（地量） | {vol_min['volume']:.0f} 手（{vol_min['date'].strftime('%Y-%m-%d')}，收盘 {vol_min['close_hfq']:.2f} 后复权） |"
        )
        lines.append("")
        ...
        # 汇报摘要
        print(
            f"当前价(不复权): {raw_close:.2f} | 后复权: {close_hfq:.2f} | 复权因子: {adj_factor:.4f}"
        )
        print(f"均线状态: {state}")
        print(f"RSI14: {rsi14:.1f}" if rsi14 is not None else "RSI14: 数据不足")
        print(
            f"ATH: {ath_row['date'].strftime('%Y-%m-%d')} {ath_row['close_hfq']:.2f}（后复权）"
        )
