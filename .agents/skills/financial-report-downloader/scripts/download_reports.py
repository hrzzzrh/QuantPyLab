...
def download_file(session, url, save_path, referer):
    try:
        ...
        if "html" in content_type:
            print(
                f"❌ Failed to download {save_path}: Received HTML instead of PDF (Anti-crawling)."
            )
            return False
...
def get_org_id(session, stock_code):
    ...
    return None, (
        ...
        else ("sse" if stock_code.startswith(("60", "68", "90")) else "szse")
    )
...
def fetch_szse_reports(stock_code, limit=12):
    ...
    session = get_session(
        {"Content-Type": "application/json", "Referer": "https://www.szse.cn/"}
    )
    payload = {
        "stock": [stock_code],
        ...
        for item in data.get("data", []):
            ...
            if is_real_report(title):
                results.append(
                    {
                        "title": title,
                        "url": "https://disc.static.szse.cn/download"
                        + item["attachPath"],
