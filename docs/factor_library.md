...
策略按 `[run].rebalance_frequency` 配置的月频、周频、双周或每 N 个交易日，在信号日收盘后计算截面分数，选择前 `holding_count` 只股票等权持有，并由现有回测引擎在下一交易日开盘成交。财务和估值字段只在这些信号日点时物化，滚动行情因子仍使用完整日频历史。配置示例：
...
`factor-composite-experiment` 是面向研发的单因子/小组合实验入口。它通过 `factor_weights` 显式选择 1 至 6 个已注册因子，使用因子元数据统一低值优先与高值优先方向，按信号日截面缩尾、百分位排名和权重合成后按配置日程等权调仓。因子专属参数放在 `factor_parameters`，例如：
