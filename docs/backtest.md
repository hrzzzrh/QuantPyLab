...
1. 信号在调仓日 T 收盘后生成。
2. T 日估值来自 `v_daily_valuation`，该视图以 `数据可用日期` ASOF 对齐，不能使用尚未全部可用的 TTM 数据。四源最早非空日期仍统一写入 `公告日期`，表示报告期首次披露口径；四源最大日期派生为 `数据可用日期`，作为安全生效边界。同步时若公告日期超过法定期限，还会通过对应交易所官方公告二次核验并覆盖该字段。
...
单元测试覆盖 TOML 配置、策略注册、质量价值与动量策略排序、T+1 成交、交易成本、缺失目标开盘价处理和行情终结（退市）清算。修改时间线、费用、复权口径、目标权重或清算逻辑时，必须先补充对应的可手算测试样例。
