...
`pub_date` 取当前 `report_date` 的四源统一公告日期，作为 TTM 估值因子的 ASOF 生效日。上年年末和上年同期仅参与当前 TTM 数值计算，不传播其历史日期；真实修订版本的严格历史可见性需要版本化财务数据模型支持。

| 序号 | 字段名 | 类型 | 单位 | 备注 | 样例值 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | report_date | VARCHAR | - | 报告期 (YYYYMMDD) | 20250930 |
| 2 | pub_date | VARCHAR | - | 当前报告期统一公告日期 (YYYYMMDD)，用于 TTM 估值 ASOF 防止未来函数 | 20251030 |
