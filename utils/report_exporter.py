...
class ReportExporter:
    ...
    def _get_sorted_files(self):
        ...
        for prefix in chapter_prefixes:
            # 查找匹配该章节前缀的文件
            matching_files = [f for f in all_files if f.startswith(prefix) and f.endswith('.md')]
            if matching_files:
                # 按文件名排序，确保稳定性
                matching_files.sort()
    ...
    def _extract_metadata(self, reports_dir):
        ...
        search_files = ["研报章节七：投资摘要与风险因素.md", "研报章节五：估值定价分析.md"]
    ...
    def export(self, output_path=None):
        ...
        with sync_playwright() as p:
            ...
            page.pdf(path=output_path, format="A4", margin={"top": "1.5cm", "bottom": "1.5cm", "left": "1.5cm", "right": "1.5cm"}, print_background=True)
            browser.close()
